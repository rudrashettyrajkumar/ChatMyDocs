...
DocChat never stores a user's provider key: the frontend keeps it in
...
(the original DocChat behaviour) with the existing per-day quotas. Any BYOK
header present → the whole chat (or embed) trio is validated here, once, so
agents downstream can trust a `RunConfig` blindly.

This module is pure parsing/validation — no I/O, no provider SDKs — which is
what keeps it trivially testable and the 400s fast (rejected before any
stream/LLM work starts, same philosophy as documents.py's validation split).
"""
...
_log = logging.getLogger("docchat.runconfig")
...
class BYOKError(ValueError):
    """A user-correctable BYOK header problem → surfaced as a 400 JSON body."""

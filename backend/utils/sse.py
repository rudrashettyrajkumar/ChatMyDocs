"""SSE formatting helpers shared by the ingestion (E2) and chat (E4) endpoints.

DocChat's SSE contract is deliberately simpler than a resumable-stream system:
there is no job queue and no cross-reconnect producer registry (ARCHITECTURE
§3 — "a queue is over-engineering at this scale"). Each `/documents` or
`/chat/stream` request drives its own generator for the lifetime of that one
HTTP connection; a client reconnect (§8: "Last-Event-ID, exponential retry")
simply retries the request from the frontend's perspective. This module only
owns:
...
Event *names* and payload shapes (`token`, `sources`, `done`, `error`, the
ingestion progress events) belong to E2/E4, not here.
"""
...
def format_token(seq: int, text: str) -> str:
    """`event: token` frame carrying a monotonically increasing `seq` (spec E4 Req 3)."""

...
No LangChain/NeMo Guardrails here (CLAUDE.md bans the framework); this module
implements a small, high-precision rail taxonomy instead:
...
* **Input rail** — `check_input()` scans the raw user message for unambiguous
  ...
  canned refusal: zero LLM tokens, no quota, and the message is NOT stored in
  history (injection text must never be replayed into a later prompt).
* **Output rail** — `guard_stream()` wraps the answerer's token stream and trips
  ...
  [HISTORY]/[QUESTION], the labels `prompt_assembly` builds in E4). Tripping
  raises `GuardrailTripped`; the SSE producer's catch-all turns that into the
  one error event, so nothing else leaks.

Bias: HIGH precision. A false positive here deflects a real person's honest
question about their document — worse than letting a clumsy injection through
to the model, whose grounding-only prompt (E4 citation_rules.md) is the second
layer. Only phrasings with no plausible innocent reading are matched: "my
manager ignores the guidelines in this report" or "does this contract override
the previous one" must pass.
...
# to DocChat's OWN instructions ("your/previous/system …"), so questions about
# document content like "does this clause override the previous section" never
# match.
_PHRASE_RULES: tuple[tuple[str, re.Pattern[str]], ...] = (
    ...
    (
        # Any mention of a system prompt, or asking the model to surface its own
        # prompt. Bare "system prompt" is high-signal — a document-QA app has no
        # innocent reading of it.
        "prompt_exfiltration",
        re.compile(
            r"\bsystem prompt\b"
...
def _responses() -> tuple[str, ...]:
    ...
    if not variants:  # a broken/empty file must never break the turn
        return ("Sorry, I can't help with that. Ask me about your uploaded documents.",)
    return variants
...
# --------------------------------------------------------------------------- output rail
# The internal prompt-block labels from utils/prompt_assembly.py (E4, ARCHITECTURE §6).
# The answerer's prose never contains bracketed markers, so any of these in the
# output means the model is echoing its own prompt structure — stop before it
# reaches the client.
...
async def guard_stream(tokens: AsyncIterator[str]) -> AsyncIterator[str]:
    ...
    (→ the one error event), so the turn degrades exactly like any other stream
    failure.
    """
    tail = ""
